...
function colorize(
  ...
  for (let i = 0; i < templatePixels.length; i += 4) {
    const templatePixel =
      ...
      templatePixels[i + 2]
    const outfitPixel =
      (outfitPixels[i] << 16) | (outfitPixels[i + 1] << 8) | outfitPixels[i + 2]
...
export async function outfit(
  ...
  let outfitCtx = outfitCanvas.getContext('2d')
  ...
  if (fileExists(outfitTemplateImagePath, outfitData)) {
    ...
    templateCtx = templateCanvas.getContext('2d') as any
    templateCtx?.drawImage(
      (await loadImage(outfitTemplateImagePath)) as any,
      0,
      0
    )
  }
  ...
  if (addons == 1 || addons == 3) {
    ...
    if (fileExists(imageFirstPath, outfitData)) {
      ...
      if (templateCtx && fileExists(imageFirstTemplatePath, outfitData)) {
        ...
        templateCtx.drawImage(imageFirstTemplate as any, 0, 0)
      }
    }
  }
  ...
  if (addons == 2 || addons == 3) {
    ...
    if (fileExists(imageSecondPath, outfitData)) {
      ...
      if (templateCtx && fileExists(imageSecondTemplatePath, outfitData)) {
        ...
        templateCtx.drawImage(imageSecondTemplate as any, 0, 0)
      }
    }
  }
  ...
  return imageOutfitTContext as any
}
